...
def exec_capture(*user_input):
	...
	user = user_input[0]
	clientID = user_input[1]
	clientSecret = user_input[2]
	user_ip = user_input[3]

	#Retrieve date to execute capture
	month = user_input[4][0:2]
	day = user_input[4][2:4]
	year = user_input[4][4:]
	...
	hour = user_input[5][0:2]
	minute = user_input[5][2:]
...
#initial message
user_input = 'Usage: camera_name clientID clientSecret ip_address date time duration'
usage = """
	Commands:
		- help: retrieve list of commands
		- exit: close the scheduler
